...
usage() {
  echo "Usage: $0 <video-url> [--frames N] [--output FILE]"
  echo ""
  echo "Options:"
  echo "  --frames N    Number of frames to extract if no transcript (default: 5)"
...
if [[ -n "$VIDEO_ID" ]]; then
  ...
  if transcript=$(get_youtube_transcript "$VIDEO_ID" 2>/dev/null); then
    ...
    log "No transcript available, falling back to frame extraction"
    VIDEO_ID=""
  fi
fi

# Fall back to frame extraction if no transcript
if [[ -z "$VIDEO_ID" ]] || [[ -z "${transcript:-}" ]]; then
  ...
  if download_video "$URL" "$VIDEO_FILE"; then
    ...
    # List frame files (for the agent to analyze)
